...
class CodelistController
{
    ...
    public function taskReindex(Event $e): void
    {
        ...
        if (!$controller->authorizeTask('reindexCodelist', ['admin.configuration', 'admin.super'])) {
            $json_response = [
                'status'  => 'error',
                'message' => '<i class="fa fa-warning"></i> '. $this->grav['language']->translate(['PLUGIN_INGRID_GRAV_UTILS.CODELIST_API.INDEXING_CODELIST_EMPTY']),
                'details' => $this->grav['language']->translate(['PLUGIN_INGRID_GRAV_UTILS.CODELIST_API.INDEXING_UNPERMISSION'])
            ];
    ...
    public static function indexJob(): array
    {
        ob_start();
        $grav = Grav::instance();
        $config = $grav['config']->get('plugins.ingrid-grav-utils.codelist_api');
    ...
    {
        $config = $this->grav['config']->get('plugins.ingrid-grav-utils.codelist_api.scheduled_index');
        if ($config['enabled']) {
            /** @var Scheduler $scheduler */
            $scheduler = $e['scheduler'];
            $at = $config['at'];
            $logs = $config['logs'];
            $job = $scheduler->addCommand('bin/plugin', ['ingrid-grav-utils', 'index-codelist'], 'ingrid-codelist-index');
            $job->at($at);
            $job->output($logs);
