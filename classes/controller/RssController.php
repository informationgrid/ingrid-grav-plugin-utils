...
class RssController
{
    ...
    public function taskReindex(Event $e): void
    {
        ...
        if (!$controller->authorizeTask('reindexRss', ['admin.configuration', 'admin.super'])) {
            $json_response = [
                'status'  => 'error',
                'message' => '<i class="fa fa-warning"></i> '. $this->grav['language']->translate(['PLUGIN_INGRID_GRAV_UTILS.RSS.INDEXING_RSS_EMPTY']),
                'details' => $this->grav['language']->translate(['PLUGIN_INGRID_GRAV_UTILS.RSS.INDEXING_UNPERMISSION'])
            ];
    ...
    public static function indexJob(): array
    {
        ...
        $output = ob_get_clean();
        [$status, $msg] = (new RssController($grav))->getCount();
    ...
    {
        $config = $this->grav['config']->get('plugins.ingrid-grav-utils.rss.scheduled_index');
        if ($config['enabled']) {
            /** @var Scheduler $scheduler */
            $scheduler = $e['scheduler'];
            $at = $config['at'];
            $logs = $config['logs'];
            $job = $scheduler->addCommand('bin/plugin', ['ingrid-grav-utils', 'index-rss'], 'ingrid-rss-index');
            $job->at($at);
            $job->output($logs);
